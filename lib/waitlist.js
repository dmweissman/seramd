...
export function applyWaitlistSignup(records, emails, rawEmail, siteEmail, options = {}) {
  const source = options.source || "coming-soon-waitlist";
  ...
  if (existing) {
    ...
    if (options.firstName) {
      existing.firstName = options.firstName;
    }
    if (options.state) {
      existing.state = options.state;
    }
    if (source === "charter") {
      existing.source = "charter";
    }
